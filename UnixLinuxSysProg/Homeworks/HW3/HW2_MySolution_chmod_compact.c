...
mode_t get_mode_condition(const char *mode_str)
{
...
int set_entered_mode(const char *mode_str, const char *const *filenames)
{
    ...
    mode_t mode_condition;
    if ((mode_condition = get_mode_condition(mode_str)) == -1)
        return -1;
    ...
    if ((ptr = strchr(mode_str, '+')))
        for (size_t i = 0; filenames[i]; ++i)
        {
            ...
            if (chmod(filenames[i], finfo.st_mode | mode_condition) == -1)
                return -1;
        }
    else if ((ptr = strchr(mode_str, '-')))
        for (size_t i = 0; filenames[i]; ++i)
        ...
        for (size_t i = 0; filenames[i]; ++i)
        {
            if (chmod(filenames[i], mode_condition) == -1)
                return -1;
        }
    else
    {
